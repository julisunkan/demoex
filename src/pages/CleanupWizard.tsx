...
import { Check, ChevronLeft, ChevronRight, Trash2, Loader2, AlertTriangle } from "lucide-react";
...
const MOCK_PREVIEW: EmailPreview[] = [
  { id: "1", subject: "Weekly Newsletter — Tech Digest",        from: "newsletter@techdigest.io", date: "Jun 20", size: "84 KB", folder: "Inbox", reason: "Newsletter" },
  { id: "2", subject: "Your Amazon order has shipped!",         from: "ship@amazon.com",          date: "Jun 15", size: "42 KB", folder: "Inbox", reason: "Promotional" },
  { id: "3", subject: "RE: Meeting tomorrow",                   from: "alice@work.com",            date: "Jan 12", size: "18 KB", folder: "Inbox", reason: "Old (>6 months)" },
  { id: "4", subject: "LinkedIn: You have 3 new connections",   from: "noreply@linkedin.com",     date: "Jun 22", size: "36 KB", folder: "Inbox", reason: "Social" },
  { id: "5", subject: "Your statement is ready",                from: "noreply@bank.com",          date: "May 1",  size: "24 KB", folder: "Inbox", reason: "Old (>6 months)" },
  { id: "6", subject: "SALE: 50% off this weekend only!",       from: "deals@shop.com",           date: "Jun 25", size: "92 KB", folder: "Inbox", reason: "Promotional" },
  { id: "7", subject: "Facebook: John liked your photo",        from: "noreply@facebook.com",     date: "Jun 18", size: "28 KB", folder: "Inbox", reason: "Social" },
  { id: "8", subject: "DUPLICATE: RE: Project update",          from: "bob@work.com",              date: "Jun 10", size: "14 KB", folder: "Inbox", reason: "Duplicate" },
];
...
export default function CleanupWizard({ isPro }: { isPro: boolean }) {
  ...
  const [selected, setSelected] = useState<Set<string>>(new Set(MOCK_PREVIEW.map(e => e.id)));
  ...
  const filteredPreview = MOCK_PREVIEW.filter(e => {
    if (!newsletters && e.reason === "Newsletter")  return false;
    if (!promotional && e.reason === "Promotional") return false;
    if (!social      && e.reason === "Social")      return false;
    if (!duplicates  && e.reason === "Duplicate")   return false;
    return true;
  });

  const selectedEmails = filteredPreview.filter(e => selected.has(e.id));
  const totalSizeMB = selectedEmails.length * 42;
  ...
  if (done) return (
    <div className="p-6 text-center space-y-4 animate-fade-in-up">
      ...
      <button onClick={() => { setDone(false); setStep(1); setConfirm(false); }} className="w-full bg-primary text-white font-bold py-2.5 rounded-xl text-sm">
        New Cleanup
      </button>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <div className="space-y-3 animate-fade-in-up">
            ...
            <div className="space-y-2">
              <div>
                ...
                <Input type="number" value={olderThan} onChange={e => setOlderThan(e.target.value)} className="text-xs mt-1" />
              </div>
              <div>
              ...
              {[
                { id: "news", label: "Newsletters",         desc: "Unsubscribe & cleanup newsletters",  state: newsletters,  set: setNewsletters, pro: false },
                ...
                { id: "dup",  label: "Duplicate Emails",    desc: "Find and remove duplicate messages", state: duplicates,   set: setDuplicates,  pro: true  },
        ...
        {step === 2 && (
          <div className="space-y-3 animate-fade-in-up">
            <div className="flex items-center justify-between">
              <p className="text-xs font-bold">{filteredPreview.length} emails found</p>
              <div className="flex items-center gap-2">
                <p className="text-[10px] text-muted-foreground">~{totalSizeMB} MB</p>
                <button onClick={() => {
                  const allIds = new Set(filteredPreview.map(e => e.id));
                  setSelected(prev => prev.size === filteredPreview.length ? new Set() : allIds);
                }} className="text-[10px] text-primary font-bold">
                  {selected.size === filteredPreview.length ? "Deselect All" : "Select All"}
                </button>
              </div>
            </div>
              ...
              {filteredPreview.map(email => (
                <label key={email.id} className={`flex items-start gap-2.5 p-2.5 rounded-xl border cursor-pointer transition-all ${
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-muted-foreground truncate">{email.from} · {email.date} · {email.size}</p>
          ...
          <div className="space-y-3 animate-fade-in-up">
            <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-4 text-center space-y-2">
              ...
              <p className="text-xs text-red-700">
                <strong>{selected.size} emails</strong> will be permanently deleted.<br />
                This frees approximately <strong>{totalSizeMB} MB</strong> of storage.
              </p>
            </div>
            {backupFirst && (
            ...
            <label className="flex items-start gap-3 p-3 rounded-xl border-2 border-border bg-white cursor-pointer">
              <input
                ...
                className="mt-0.5 accent-red-600 shrink-0"
              />
              <p className="text-xs">I understand this will permanently delete {selected.size} emails and this cannot be undone.</p>
            </label>
          </div>
        )}
      </div>
      ...
      <div className="border-t bg-white p-3 flex gap-2 shrink-0">
        ...
        {step < TOTAL ? (
          <button onClick={() => setStep(s => s + 1)} className="flex-1 flex items-center justify-center gap-1 bg-red-600 text-white py-2.5 rounded-xl text-xs font-bold hover:bg-red-700">
            Next <ChevronRight className="w-3.5 h-3.5" />
          </button>
        ) : (
          <button onClick={runCleanup} disabled={!confirm || selected.size === 0} className="flex-1 flex items-center justify-center gap-2 bg-red-600 text-white py-2.5 rounded-xl text-sm font-black hover:bg-red-700 disabled:opacity-40">
