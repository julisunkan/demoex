...
import { restoreSession, signInPopup, getAccessToken, checkSubscription, initializeMsal, type SubscriptionInfo } from "./lib/auth";
...
export default function App() {
  ...
  useEffect(() => {
    // Initialize MSAL first, then attempt silent session restore
    initializeMsal()
      .then(() => restoreSession())
      .then((session) => { if (session?.subscription.subscribed) setIsPro(true); })
      .catch(() => {/* no cached session — normal on first load */});
  }, []);
  ...
  const onSubscriptionUnlocked = useCallback((_sub: SubscriptionInfo) => {
    setIsPro(true);
    setShowSignIn(false);
  ...
  return (
    <div className="flex flex-col h-screen bg-background text-foreground">
      {/* Microsoft sign-in / subscription gate */}
      {showSignIn && (
        <SignInGate
          onUnlocked={onSubscriptionUnlocked}
          onDismiss={() => { setShowSignIn(false); setPendingAction(null); }}
        />
      )}
                  ...
                  {summary.duplicateRows.size > 0 && (
                    <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
                      ...
                      {isPro
                        ...
                        : <ProBadge onClick={() => setShowSignIn(true)} />
                      }
                    </div>
                  )}
